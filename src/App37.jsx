...
function Child11(){
  ...
  useEffect(() => {
    console.log("첫 랜더링시 실행 (mount 시 실행)");

    // unmount 때 실행되는 코드
    // : ueeEffect의 첫번째 파라미터로 들어가는 함수의 리턴되는 함수
    return () => {
...
function Child21() {
  // 연습 :
  // 이 컴포넌트가 마운트 될 때 로그로 "자식 21 마운트됨" 출력
  // 이 컴포넌트가 언마운트 될 때 로그로 "자식 21 언마운트됨" 출력
...
function Child31() {
  ...
  // useEffect의 두번째 파라미터 : dependencies(deps)
  // 변경 감지할 값들의 목록
  // 이 목록의 값이 변경되면 useEffect의 첫번째 파라미터(함수) 실행
  useEffect(() => {
    // 마운트외 업데이트 둘다 실행됨
    console.log("업데이트되면 시행되는 함수의 코드들...")
  }, [count]);
  ...
  useEffect(() => {
    console.log("count/otherCont 업데이트")
  }, [count,otherCount]);
  return (
    <div>
      <button onClick={() => setCount(count + 1)}>카운트 {count}</button>
      <button onClick={() => setOtherCount(otherCount + 1)}>다른 카운트 {otherCount}</button>
    </div>
  )
}
